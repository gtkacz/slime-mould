...
- $\vec{v_b} \in [-a, a]$, em que $a = \mathrm{arctanh}(\frac{1-t}{I_{\max}})$.
...
\vec{X^{*}} =
\begin{cases}
\text{rand} \cdot (UB - LB) + LB, & \text{rand} < \zeta \\[4pt]
...
- Três casos, **por indivíduo, por iteração**:
  1. ***ζ-branch*** (prob. $\zeta = 0.03$): reinício aleatório no espaço de busca.
...
A *ζ-branch* permite que o SMA escape de ótimos locais sem influência externa explícita e sem reiniciar a população inteira.

</div>

</div>

<p class="citation">Li et al. (2020), §2.3.2 Eq. (2.7); $\zeta$ escolhido como 0.03 a partir do <em>sweep</em> de sensibilidade §3.4.</p>
...
- Três hiperparâmetros: população $N$, iterações $I_{\max}$, probabilidade de reinício $\zeta$. Os valores de $v_b, v_c$ são derivados.
...
Em seguida, levamos a <strong>dinâmica de atualização</strong> do SMA (evolução de $v_b$ e $v_c$, pesos assinados por *ranking* e *ζ-restart*) para o feromônio, não para uma coordenada.
...
```text
Entrada (grade + K pontos) -> Checagem prévia O(N²) -> Código Python popualação x I_max -> Avaliação + registro
```
...
| $v_b \sim \mathcal{U}(-a,a)$ **por agente** ($a = \mathrm{arctanh}(\frac{1-t}{I_{\max}})$): oscilador **estocástico**, ilimitado em $t=0$ | $v_b = \tanh(1 - \frac{t}{I_{\max}})$: escalar **determinístico** por iteração, em $[0, \tanh 1]$ | Saturação estabiliza depósitos discretos; a perda da oscilação estocástica de $v_b$ é compensada pela amostragem *softmax* dos *walkers* |
| Atualização com *switching* em três casos (ζ / *approach* / *oscillate*) | Soma **única** $v_c\tau + v_b\Delta$ + ruído *ζ-branch* | Todos os ingredientes de Li em cada passo; sem sorteio de *branch* por indivíduo |
